#!/bin/bash
# https://github.com/ederitsolutions/nwmagentapmx
...
REPO=${REPO:-"ederitsolutions/nwmagentapmx"}
DEFAULT_TAG="master"
TAG=${TAG:-$DEFAULT_TAG}
...
function checkSupported {   
    ...
    if [ -z "$SUPPORTED" ]; then 
        ...
        if ! (printf '%s\n' "${SUPPORTEDARR[@]}" | grep -q -P "$PVEVersionMajor"); then
            ...
            echo -e "If you do run into any issues on >newer< versions, please consider opening an issue at https://github.com/ederitsolutions/nwmagentapmx/issues.${REG}"
        fi
    fi
}
...
function usage {
    if [ "$_silent" = false ]; then
        ...
        echo -e "Manages the nwmagentapmx theme."
    ...
    #    echo -e "  utility-update       Update this utility\n" (to be implemented)
        ...
        echo -e "Report issues at: <https://github.com/ederitsolutions/nwmagentapmx/issues>"
    fi
}
...
function install {
    if isInstalled; then
        ...
        if [ "$OFFLINE" = false ]; then
            curl -s $BASE_URL/nwmagentapmx/sass/nwmagentapmx.css > /usr/share/pve-manager/css/dd_style.css
        else
            cp "$OFFLINEDIR/nwmagentapmx/sass/nwmagentapmx.css" /usr/share/pve-manager/css/dd_style.css
        fi

        if [ "$_silent" = false ]; then echo -e "${CHECKMARK} Downloading patcher"; fi
        if [ "$OFFLINE" = false ]; then
            curl -s $BASE_URL/nwmagentapmx/js/nwmagentapmx.js > /usr/share/pve-manager/js/dd_patcher.js
        else
            cp "$OFFLINEDIR/nwmagentapmx/js/nwmagentapmx.js" /usr/share/pve-manager/js/dd_patcher.js
        fi
        ...
        do
                if [ "$OFFLINE" = false ]; then
                    curl -s $BASE_URL/nwmagentapmx/images/$image > /usr/share/pve-manager/images/$image
                else
                    cp "$OFFLINEDIR/nwmagentapmx/images/$image" /usr/share/pve-manager/images/$image
                fi
                ((ITER++))
